...
- **Service worker** (`web/static/sw.js`, served from `/sw.js`): aggressively caches static assets, CDN libs, OSM tiles, and `/api/*` JSON (stale-while-revalidate) so the app survives flaky Wendland mobile reception. Registers on every origin the browser allows it on (HTTPS or `localhost`/`127.0.0.1`). For dev iteration, use DevTools → Application → Service Workers → "Bypass for network" / "Update on reload", or bump `VERSION` in `sw.js` to force-invalidate user caches on deploy.
