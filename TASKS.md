...
### 36. ~~Missing Postal Codes Prevent Geocoding~~ ✅ COMPLETED (2026-01-29)
...
- **Solution**: Added Google Maps Geocoding API support
  - Google Maps can geocode venues with just city + street (postal code optional)
  - Added `--google-api-key` flag to crawler command
  - Changed venue validation to require city OR street (not postal code)
  - Geocoding logic checks provider: Nominatim still requires postal code, Google Maps doesn't
  - Falls back to city/street when postal code missing
- **Usage**:
  ```bash
  go run cmd/crawler/main.go --google-api-key YOUR_API_KEY
  ```
- **Files**:
  - `internal/crawler/geocoder.go` - added NewGoogleMapsGeocoder() and geocodeGoogle()
  - `internal/crawler/scraper.go` - updated GeocodeVenues() logic, added SetGoogleMapsGeocoder()
  - `internal/storage/models.go` - relaxed postal code validation
  - `cmd/crawler/main.go` - added --google-api-key flag
- **Priority**: ~~High~~ DONE
